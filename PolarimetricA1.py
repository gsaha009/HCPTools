...
class PolarimetricA1:
    ...
    def printinfo(self, p4, type="", plot=True):
        if type=="p":
            print(f"    px: {p4.px}")
            print(f"    py: {p4.py}")
            print(f"    pz: {p4.pz}")
    ...
    def Rotate(self, vect, rotvect):
        ...
        vect = self.RotateZ(vect, (0.5*np.pi - rotvect.phi))
        vect = self.RotateX(vect, rotvect.theta)
    ...
    def PVC(self):
        ...
        self.plotit(arrlist=[ak.ravel(s1).to_numpy(),
        ...
        #getvec = lambda a,b,c: b - c - a*(b - c)*(1/a.mass2)
        ...
        self.plotit(arrlist=[ak.ravel(F1.Re()).to_numpy(),
        ...
        self.plotit(arrlist=[ak.ravel(F2.Re()).to_numpy(),
        ...
        self.plotit(arrlist=[ak.ravel(F3.Re()).to_numpy(),
        ...
        """
        print(f"  HC_E: \n  Re: {HADCUR['HC_E'].Re()}\n  Im: {HADCUR['HC_E'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCUR['HC_E'].Re()).to_numpy(),
                             ak.ravel(HADCUR['HC_E'].Im()).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,2))
        print(f"  HC_Px: \n  Re: {HADCUR['HC_Px'].Re()}\n  Im: {HADCUR['HC_Px'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCUR['HC_Px'].Re()).to_numpy(),
                             ak.ravel(HADCUR['HC_Px'].Im()).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,2))
        print(f"  HC_Py: \n  Re: {HADCUR['HC_Py'].Re()}\n  Im: {HADCUR['HC_Py'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCUR['HC_Py'].Re()).to_numpy(),
                             ak.ravel(HADCUR['HC_Py'].Im()).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,2))
        print(f"  HC_Pz: \n  Re: {HADCUR['HC_Pz'].Re()}\n  Im: {HADCUR['HC_Pz'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCUR['HC_Pz'].Re()).to_numpy(),
        ...
        print(f"  C-HC_E: \n  Re: {HADCURC['HC_E'].Re()}\n  Im: {HADCURC['HC_E'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCURC['HC_E'].Re()).to_numpy(),
                             ak.ravel(HADCURC['HC_E'].Im()).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,2))
        print(f"  C-HC_Px: \n  Re: {HADCURC['HC_Px'].Re()}\n  Im: {HADCURC['HC_Px'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCURC['HC_Px'].Re()).to_numpy(),
                             ak.ravel(HADCURC['HC_Px'].Im()).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,2))
        print(f"  C-HC_Py: \n  Re: {HADCURC['HC_Py'].Re()}\n  Im: {HADCURC['HC_Py'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCURC['HC_Py'].Re()).to_numpy(),
                             ak.ravel(HADCURC['HC_Py'].Im()).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,2))
        print(f"  C-HC_Pz: \n  Re: {HADCURC['HC_Pz'].Re()}\n  Im: {HADCURC['HC_Pz'].Im()}")
        self.plotit(arrlist=[ak.ravel(HADCURC['HC_Pz'].Re()).to_numpy(),
        ...
        self.plotit(arrlist=[ak.ravel(pclv).to_numpy(),
    ...
    def F3PI(self,
        ...
        #TComplex(1, 0.99*TMath::Pi(), true);   Real part must be equal to one, stupid polar implemenation in root
        ...
        if IDK == 1:
            ...
            if IFORM == 1 or IFORM == 2:
                ...
                print("  === FRO1 ===>")
                FRO1 = self.BWIGML(S1, MRO, GRO, M2, M3, 1)
                print(f"  === FRO1: {FRO1.Re()}\t{FRO1.Im()}\n")

                print("  === FRP1 ===>")
                FRP1 = self.BWIGML(S1, MRP, GRP, M2, M3, 1)
                print(f"  === FRP1: {FRP1.Re()}\t{FRP1.Im()}\n")

                print("  === FRO2 ===>")
                FRO2 = self.BWIGML(S2, MRO, GRO, M3, M1, 1)
                print(f"  === FRO2: {FRO2.Re()}\t{FRO2.Im()}\n")
                
                print("  === FRP2 ===>")
                FRP2 = self.BWIGML(S2, MRP, GRP, M3, M1, 1)
                print(f"  === FRP2: {FRP2.Re()}\t{FRP2.Im()}\n")

                print("  === FF23 ===>")
                FF23 = self.BWIGML(S3, MF2, GF2, M1, M2, 2)
                print(f"  === FF23: {FF23.Re()}\t{FF23.Im()}\n")

                print("  === FSG3 ===>")
                FSG3 = self.BWIGML(S3, MSG, GSG, M1, M2, 0)
                print(f"  === FSG3: {FSG3.Re()}\t{FSG3.Im()}\n")

                print("  === FF03 ===>")
                FF03 = self.BWIGML(S3, MF0, GF0, M1, M2, 0)
                print(f"  === FF03: {FF03.Re()}\t{FF03.Im()}\n")
                ...
                F3PIFactor = BT1*FRO1 \
                           ...
                           + BT6*TComplex(F167, 0.)*FSG3 \
                           + BT7*TComplex(F167, 0.)*FF03
                print(f"  F3PIFactor: {F3PIFactor.Re()}\t{F3PIFactor.Im()}\n\n")
    ...
    def GetWGS(self, S: ak.Array, MP: float, MM: float, MSQ: float, L: int,
        ...
        print(f" --- GetWGS --- : --- Start --- : --- calling from BWIGML ---")
    ...
    def BWIGML(self,
               ...
               m2: float,
               L: int):
        print(f" --- BWIGML --- : --- Start --- : --- calling from F3PI --- ")
        ...
        print(f"  WGS: {WGS}")
        self.plotit(arrlist=[ak.ravel(WGS).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,1))
        out = TComplex(MSQ, 0)/TComplex(MSQ - S, -WGS)
        print(f"  out: {out.Re()}\t{out.Im()}")
        self.plotit(arrlist=[ak.ravel(out.Re()).to_numpy(),
    ...
    def FA1A1P(self, XMSQ: ak.Array):
        ...
        F1 = TComplex(-XM1SQ, 0.0)/TComplex(XMSQ - XM1SQ, FG1)
        F2 = TComplex(-XM2SQ, 0.0)/TComplex(XMSQ - XM2SQ, FG2)
        FA1A1P = F1 + (BET*F2)
    ...
    def Widths(self, Q, part_type):
        ...
        print(f"  Gamma: {Gamma}")
        self.plotit(arrlist=[ak.ravel(Gamma).to_numpy()],
                    bins=100,
                    log=False,
                    dim=(1,1))
        print(" ----- Widths --- : --- End ---")
    ...
    def ga1(self, Q):
        ...
        if isinstance(Q, (int, float)):
            ...
            self.plotit(arrlist=[ak.ravel(out).to_numpy()],
                        bins=100,
                        log=False,
                        dim=(1,1))
